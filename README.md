...
- Tables, charts, SmartArt, equations, grouped text, and text with unsupported shape-level effects remain unchanged and are reported for review.
...
- 复杂的逐字、逐词或逐段动画不会转换。
- 表格、图表、SmartArt、公式、组合内文字以及带有不支持形状级特效的文字会保持原状，并列入复核报告。
